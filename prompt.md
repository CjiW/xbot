...
各 memory block 有硬性字符限制和推荐长度：

| Block | 硬性限制 | 推荐长度 | 说明 |
|-------|---------|---------|------|
| **persona** | 2000 字符 | 500-1500 字符 | 身份、性格、价值观 |
| **human** | 2000 字符 | 300-1000 字符 | 每用户关键特征 |
| **working_context** | 4000 字符 | 500-2000 字符 | 当前任务、活跃事实 |
| **history_entry** | 无限制 | 50-200 字符 | [YYYY-MM-DD HH:MM] 开头 |
| **archival_entries** | 无限制 | 100-500 字符/条 | 含可搜索关键词 |

**注意**：超出硬性限制会写入失败。推荐长度为最佳实践，超长会挤占上下文窗口。
