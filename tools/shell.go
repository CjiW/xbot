...
func (t *ShellTool) executeForeground(
	...
	// 检测 export 命令并持久化环境变量
	// All sandboxes (including none) need persistence because each command
	// runs in a fresh process — environment variables don't survive across calls.
...
func (t *ShellTool) persistEnvFromCommand(toolCtx *ToolContext, command string) bool {
	...
	lines = append(lines, "# This file is sourced by your shell rc file")
	...
	// Ensure the user's shell rc file sources ~/.xbot_env.
	// Detects current shell from $SHELL and injects into the correct rc:
	//   bash → ~/.bashrc (before PS1 guard)
	//   zsh  → ~/.zshrc
	//   other → ~/.profile
	ensureRcCmd := `shell_name=$(basename "${SHELL:-/bin/sh}")
...
# Remove existing source block
if grep -q 'source ~/.xbot_env' "$rc_file" 2>/dev/null; then
    sed -i '/# Source xbot environment variables/,/source ~\/\.xbot_env/d' "$rc_file"
    sed -i '/^$/{ N; /^\n$/d; }' "$rc_file"
fi
