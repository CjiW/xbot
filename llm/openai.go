...
func NewOpenAILLM(cfg OpenAIConfig) *OpenAILLM {
	...
	o.SetDefaultModel(cfg.DefaultModel)
	...
	if err := o.LoadModelsFromAPI(ctx); err != nil {
		...
		// API 获取失败，使用默认模型作为回退
		if cfg.DefaultModel != "" {
			o.SetModels([]string{cfg.DefaultModel})
...
func (o *OpenAILLM) LoadModelsFromAPI(ctx context.Context) error {
	...
	// 如果没有设置默认模型，使用第一个模型
	if o.GetDefaultModel() == "" && len(o.ListModels()) > 0 {
		o.SetDefaultModel(o.ListModels()[0])
	}

	log.WithFields(log.Fields{
		"model_count":   len(o.ListModels()),
		"default_model": o.GetDefaultModel(),
