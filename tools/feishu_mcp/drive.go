...
func (t *UploadFileTool) Description() string {
	return "Upload a file to the user's Feishu cloud space. " +
		...
		"STEP 2: Then call this tool with the file path."
}
...
func (t *ListFilesTool) Description() string {
	return "List files and folders in a Feishu cloud space folder. " +
		"STEP 1: First call oauth_authorize with provider='feishu' if not already authorized. " +
		"STEP 2: Then call this tool with optional folder_token."
}
...
func (t *ListFilesTool) Execute(ctx *tools.ToolContext, input string) (*tools.ToolResult, error) {
	...
	return tools.NewResult(
		"File listing requires using specific folder tokens.\n\n" +
			"To get file/folder tokens:\n" +
			"- Navigate to a folder in Feishu\n" +
			"- Copy the token from the URL (e.g., https://xxx.feishu.cn/drive/folder/TOKENXXXX)\n" +
			"- Use feishu_upload_file to upload files\n" +
			"- Use feishu_add_permission to share files with others",
	), nil
}
...
func (t *AddPermissionTool) Description() string {
	return "Add a collaborator (permission) to a file or folder. " +
		"STEP 1: First call oauth_authorize with provider='feishu' if not already authorized. " +
		"STEP 2: Then call this tool with token, member info, and permission level."
}
